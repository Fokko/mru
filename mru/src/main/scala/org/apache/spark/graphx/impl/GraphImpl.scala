...
class GraphImpl[VD: ClassTag, ED: ClassTag] protected (
  ...
  override def mapReduceUpdate[M: ClassTag](
      ...
      reduceF: (M, M) => M,
      updateF: (VertexId, VD, Option[M]) => VD)
    : Graph[VD, ED] =
  { 
    val srcAttr = accessesVertexAttr(mapF, "srcAttr")
    val dstAttr = accessesVertexAttr(mapF, "dstAttr")
  
    val vs = replicatedVertexView.get(srcAttr, dstAttr)
    
    val globalMess = edges.partitionsRDD.zipPartitions(vs, true) { (edgePartitionIter, vertexPartitionIter) =>
      val (edgePid, edgePartition) = edgePartitionIter.next()
      val (vertexPid, vertexPartition) = vertexPartitionIter.next()
      val et = new EdgeTriplet[VD, ED]
      val edgePartMess = edgePartition.iterator.flatMap { e =>
        et.set(e) 
        if (srcAttr) {
       ...
       vertexPartition.aggregateUsingIndex(edgePartMess, reduceF).iterator
    }
    
    val t = vertices.aggregateUsingIndex(globalMess, reduceF)
    val newVerts = vertices.leftJoin(t)(updateF)
    val changedVerts = vertices.asInstanceOf[VertexRDD[VD]].diff(newVerts)
    val newReplicatedVertexView = new ReplicatedVertexView[VD](
      changedVerts, edges, routingTable,
